...
const SignUpEmail = () => {
  ...
  const [email, setEmail] = useState<string>('');
    ...
    <View style={className` ${ colorScheme === 'light' ? 'bg-[#f7f7f7]' : 'bg-[#0e1a32]'} h-screen flex-1 `}>
        ...
        <View style={className` flex-row mx-4 p-2 items-center overflow-hidden rounded-xl  ${ colorScheme === 'light' ? `${ isInputErr ? 'border border-red-500' : 'border border-[#0261ef]' } bg-[#e6edfd] ` : `${ isInputErr ? 'border border-red-500' : 'border border-[#ffd75b]'  } bg-[#1a263e] `}`}>
